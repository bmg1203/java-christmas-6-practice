...
* 주문 메뉴
  * 주문 메뉴와 개수를 작성합니다. 
  * `해산물파스타-2,레드와인-1`의 형태로 입력한다. 

### 유효성 검사
> 유효하지 않은 경우 IllegalArgumentException 에러를 발생시킨다. 
> "[ERROR]"가 포함된 에러를 안내하고 그부분부터 다시 입력 받는다. 
* 예약 날짜
  * 1~31 사이의 숫자가 아닌 경우
  * 숫자가 아닌 경우
  * 입력이 없는 경우
* 주문 메뉴
  ...
  * 주문 개수가 20개 초과인 경우
  * 주문 개수가 0인 경우
  * 주문 메뉴 총주문 금액이 10,000원 이하인 경우

### 메뉴 저장 기능
* 존재하는 메뉴를 저장 합니다. 
* 에피타이저, 메인, 디저트, 음료가 존재합니다. 

### 크리스마스 디데이 할인 기능
* 12/1~25인 경우 10,000원 이상 구매시 적용됩니다. 
* 날짜에 따라 1,000원부터 시작되어 100원씩 추가 할인됩니다. 

### 평일 or 주말 할인 기능
* 12월 내내 10,000원 이상 구매시 적용됩니다. 
* 평일 할인
  * 일~목요일에 적용됩니다. 
  * 디저트 메뉴 개수당 2,023원 할인됩니다. 
* 주말 할인
  * 금,토요일에 적용됩니다. 
  * 메인 메뉴 개수당 2,023원 할인됩니다. 

### 특별 할인 기능
* 12월 내내 10,000원 이상 구매시 적용됩니다.
* 별이 달린 3,10,17,24,31일에 적용됩니다. 
* 총주문 금액에서 1,000원 할인 됩니다. 

### 증정 이벤트 기능
* 12월 내내 10,000원 이상 구매시 적용됩니다. 
* 할인 전 총주문 금액이 12만원 이상일 때, 샴페인 1개 증정됩니다. 
...
### 고객 이벤트 주의 사항 안내(에러 메시지)
* 총주문 금액 10,000원 미만인 경우 이벤트 미적용 안내문
* 음료만 주문시 주문 불가 안내문

### 가격 계산 기능
* 총주문 가격을 계산합니다. 
* 할인 혜택을 적용한 가격을 계산합니다. 